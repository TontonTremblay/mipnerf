  ...
  for split in splits:
    ...
    for i, img in enumerate(data['images']):
      for j in range(n_down):
        ...
        labels.append(j)
        nears.append(near_far[i][0])
        fars.append(near_far[i][1])
        img = down2(img)
...
def main(unused_argv):
  ...
  far = -1000000000
  per_image_near_far = {}
  for i_image, image in enumerate(sorted(glob.glob(os.path.join(blenderdir+"/*.depth.exr")))):
    depth = cv2.imread(image,  
            cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH) 
    depth[depth>3.4028235e+37] = 0
    depth[depth<-3.4028235e+37] = 0 

    per_image_near_far[i_image] = [
              depth[depth>0].min()-depth[depth>0].min()*0.9,
              depth[depth>0].max()+depth[depth>0].max()*0.9
            ]
  ...
  for basedir in dirs:
    ...
    convert_to_nerfdata(outdir, outdir, n_down, near_far = per_image_near_far)
